...
import { ALL_SCENARIOS, getScenarioById, getScenariosByCategory } from "@/test/scenarios"
import { mockRetrieve, mockDeterministicRetrieval, getDeterministicResponse } from "@/test/mocks"
...
import { generateMarkdownReport, calculateAggregateStats } from "@/test/reports/report-generator"
...
export async function evaluateScenario(
  ...
  const expectedChunkIds = scenario.expectedRetrievedContext.map((c) => c.chunkId)
...
export async function runFullEvaluation(
  ...
  console.log(`  Similarity           : ${aggregateStats.categoryAverages.similarity}`)
  console.log(`  Relevance            : ${aggregateStats.categoryAverages.relevance}`)
  console.log(`  Empathy              : ${aggregateStats.categoryAverages.empathy}`)
  console.log(`  Contextual Consistency: ${aggregateStats.categoryAverages.contextualConsistency}`)
  console.log(`  Retrieval Accuracy   : ${aggregateStats.categoryAverages.retrievalAccuracy}`)
