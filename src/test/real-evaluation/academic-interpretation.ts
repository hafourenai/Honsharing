...
function generateFailureInterpretation(failures: FailureAnalysis[]): string {
  ...
  return (
    ...
    `1. **Respons Tidak Relevan**: ${irrelevantCount} kasus — Respons chatbot tidak ` +
    `memiiki keterkaitan dengan input pengguna.\n` +
    `2. **Retrieval Salah Konteks**: ${wrongContextCount} kasus — Sistem RAG mengambil ` +
    `chunk yang tidak sesuai dengan query.\n` +
    `3. **Respons Generik**: ${genericCount} kasus — Chatbot memberikan jawaban yang ` +
    `terlalu umum dan tidak spesifik.\n` +
    `4. **Halusinasi**: ${hallucinationCount} kasus — Chatbot memberikan informasi yang ` +
    `tidak akurat atau tidak seharusnya.\n` +
    `5. **Mismatch Emosional**: ${mismatchCount} kasus — Nada emosional respons tidak ` +
