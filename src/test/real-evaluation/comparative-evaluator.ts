/**
 * ============================================================
 * COMPARATIVE EVALUATOR — PERBANDINGAN RAG VS NON-RAG (REAL)
 * ============================================================
 *
 * Membandingkan respons chatbot dengan dan tanpa retrieval
 * context, menggunakan evaluator kualitas yang sudah ada.
 *
 * TUJUAN PENELITIAN:
 *   Menunjukkan efektivitas RAG dalam meningkatkan:
 *   1. Contextual quality — kesesuaian konteks
 *   2. Empathy — tingkat empati
 *   3. Specificity — kekhususan respons
 *   4. Emotional consistency — konsistensi emosional
 *
 * CARA KERJA:
 *   Non-RAG: Chatbot merespon tanpa konteks retrieval
 *   RAG:     Chatbot merespon dengan konteks retrieval
 *
 * @author  Tim Skripsi
 * @version 1.0
 * ============================================================
...
export async function compareRealScenario(
  ...
  const retrievedChunks = (await mockRetrieve(scenario.userInput)).map(
    (c, i) =>
      ({
        chunkId: c.id,
...
export async function compareRealAllScenarios(
  ...
  console.log(
    `Rata-rata RAG: ${(totalContextualRag / count).toFixed(1)}`
  )
  console.log(
