/**
 * ============================================================
 * EVALUATION SESSION LOGGER — LOGGER SESI EVALUASI
 * ============================================================
 *
 * Mencatat seluruh aktivitas evaluasi untuk dokumentasi
 * dan analisis skripsi.
 *
 * FORMAT OUTPUT:
 * 1. JSON — Data mentah untuk olah data lanjutan
 * 2. Markdown — Laporan terbaca manusia
 *
 * DATA YANG DICATAT:
 * - user input
 * - retrieved context (chunk ID, similarity, rank)
 * - generated response
 * - similarity score
 * - empathy score
 * - relevance score
...
export function generateSessionMarkdown(
  ...
  lines.push(`|--------|-------|`)
  lines.push(`| Rata-rata Similarity | ${session.summary.averageSimilarity}/100 |`)
  lines.push(`| Rata-rata Empati | ${session.summary.averageEmpathy}/100 |`)
  lines.push(`| Rata-rata Relevansi | ${session.summary.averageRelevance}/100 |`)
  lines.push(`| Rata-rata Retrieval | ${session.summary.averageRetrieval}/100 |`)
  lines.push(`| Rata-rata Waktu Respons | ${session.summary.averageResponseTimeMs}ms |`)
  lines.push(``)
  lines.push(``)
  ...
  for (const label of labelOrder) {
    ...
    const pct = session.summary.totalScenarios > 0
