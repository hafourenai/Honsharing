/**
 * ============================================================
 * VISUALIZATION DATA — DATA SIAP VISUALISASI
 * ============================================================
 *
 * Modul ini menghasilkan data yang siap digunakan untuk:
 * - Diagram batang (bar chart)
 * - Diagram pie (pie chart)
 * - Diagram distribusi skor (score distribution)
 * - Diagram perbandingan (comparison chart)
 *
 * Output:
 * - JSON (untuk frontend chart library)
 * - Markdown table (untuk dokumentasi)
 * - CSV (untuk Excel/SPSS)
 *
 * TIDAK perlu frontend chart.
 * Cukup generate data yang bisa langsung diplot.
 *
 * @author  Tim Skripsi
 * @version 1.0
 * ============================================================
 ...
 */
export function barChartPerScenario(
 ...
 * @returns BarChartData
 */
export function barChartPerCategory(
  results: EvaluationResult[]
): BarChartData {
 ...
 * @returns BarChartData
 */
export function barChartPerDimension(
...
export function pieChartVerdictDistribution(
  ...
  const sortedVerdicts = ["SANGAT_BAIK", "BAIK", "CUKUP", "KURANG", "TIDAK_MEMADAI"]

  return {
    labels: sortedVerdicts.filter((v) => verdictCount[v]),
    data: sortedVerdicts.filter((v) => verdictCount[v]).map((v) => verdictCount[v]),
...
export function calculateScoreDistribution(
  ...
  const ranges = [
    { label: "0–39 (Tidak Memadai)", min: 0, max: 39 },
    { label: "40–54 (Kurang)", min: 40, max: 54 },
    { label: "55–69 (Cukup)", min: 55, max: 69 },
    { label: "70–84 (Baik)", min: 70, max: 84 },
    { label: "85–100 (Sangat Baik)", min: 85, max: 100 },
  ]
