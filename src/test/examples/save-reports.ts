/**
 * SAVE REPORTS — Simpan semua output evaluasi ke file markdown
 *
 * Cara pakai:
 *   npx tsx src/test/examples/save-reports.ts
 */
...
async function main() {
  ...
  writeFileSync(join(OUT, "01-evaluation-report.md"), markdown, "utf-8")
  console.log(`✓ 01-evaluation-report.md (${markdown.length} chars)`)

  // 2. Academic report (BAB 4)
  writeFileSync(join(OUT, "02-academic-report-bab4.md"), academicMarkdown, "utf-8")
  console.log(`✓ 02-academic-report-bab4.md (${academicMarkdown.length} chars)`)

  // 3. RAG vs Non-RAG comparison
  writeFileSync(join(OUT, "03-rag-vs-non-rag.md"), comparisonTable, "utf-8")
  console.log(`✓ 03-rag-vs-non-rag.md (${comparisonTable.length} chars)`)
...
\`\`\`
`
  writeFileSync(join(OUT, "04-full-results.md"), combined, "utf-8")
  console.log(`✓ 04-full-results.md (${combined.length} chars)`)

  // 5. JSON data
  writeFileSync(join(OUT, "05-statistical-summary.json"), JSON.stringify(jsonSummary, null, 2), "utf-8")
  console.log(`✓ 05-statistical-summary.json`)

  // 6. CSV
  writeFileSync(join(OUT, "06-evaluation-data.csv"), csvData, "utf-8")
  console.log(`✓ 06-evaluation-data.csv (${csvData.split("\n").length} rows)`)
