...
function generateAnalisisHasil(report: EvaluationReport): string {
  ...
  if (dims.empathy >= 75) {
    ...
    lines.push(``);
    lines.push(
      `1. **Memvalidasi emosi pengguna** — Menggunakan kata-kata seperti "wajar", "paham", "dimengerti"`,
    );
    lines.push(
      `2. **Menunjukkan pemahaman** — Merefleksikan kembali perasaan pengguna dengan nada yang hangat`,
    );
    lines.push(
      `3. **Memberikan dukungan** — Menawarkan kehadiran dan dukungan emosional yang tulus`,
    );
    lines.push(``);
  } else {
    lines.push(
