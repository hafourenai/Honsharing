/**
 * ============================================================
 * ACADEMIC REPORT GENERATOR — LAPORAN AKADEMIK BAHASA INDONESIA
 * ============================================================
 *
 * Modul ini menghasilkan laporan evaluasi dalam format akademik
 * berbahasa Indonesia formal, yang siap digunakan untuk BAB 4
 * skripsi (Hasil dan Pembahasan).
 *
 * FITUR:
 * 1. Tujuan pengujian
 * 2. Metode evaluasi
 ...
 * 4. Tabel hasil
 * 5. Analisis hasil
 * 6. Interpretasi akademik
 * 7. Kesimpulan pengujian
 *
 * FORMAT:
 * - Markdown dengan gaya bahasa formal Indonesia
 * - Struktur sesuai pedoman penulisan skripsi
 * - Tabel dan statistik yang jelas
 * - Interpretasi yang siap presentasi saat sidang
 *
 * @author  Tim Skripsi
 * @version 1.0
 * ============================================================
...
import { calculateCategoryStats, calculateSuccessRate, calculateVerdictDistribution } from "@/test/statistics/statistical-summary"
...
function generateHasilPengujian(report: EvaluationReport): string {
  ...
  lines.push(``)
  lines.push(`| Kategori | Jumlah | Rata-rata | Similarity | Relevance | Empathy | Consistency | Retrieval |`)
  lines.push(`|----------|--------|-----------|------------|-----------|---------|-------------|-----------|`)
  ...
  lines.push(``)
  lines.push(`| # | Skenario | Kategori | Sim | Rel | Emp | Kon | Ret | Overall |`)
  lines.push(`|---|----------|----------|-----|-----|-----|-----|-----|---------|`)
...
function generateAnalisisHasil(report: EvaluationReport): string {
  ...
  if (dims.empathy >= 75) {
    ...
    lines.push(``)
    lines.push(`1. **Memvalidasi emosi pengguna** — Menggunakan kata-kata seperti "wajar", "paham", "dimengerti"`)
    lines.push(`2. **Menunjukkan pemahaman** — Merefleksikan kembali perasaan pengguna dengan nada yang hangat`)
    lines.push(`3. **Memberikan dukungan** — Menawarkan kehadiran dan dukungan emosional yang tulus`)
    lines.push(``)
  } else {
    lines.push(
  ...
  const sortedCatStats = [...catStats].sort((a, b) => b.averageScore - a.averageScore)
