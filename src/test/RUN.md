...
> **Sebelum jalan:** Buka `src/test/examples/usage-example.ts`, cari baris:
> ```ts
...
File: `src/test/examples/full-evaluation-runner.ts`
...
- Report tersimpan di: `src/test/generated-reports/`
...
| File | Fungsi |
|------|--------|
| `src/test/examples/usage-example.ts` | Contoh lengkap semua evaluasi |
| `src/test/examples/full-evaluation-runner.ts` | Runner evaluasi penuh |
| `src/test/examples/save-reports.ts` | Simpan report ke file |
| `src/test/runner/evaluation-runner.ts` | Logic evaluasi |
| `src/test/config/test-config.ts` | Konfigurasi threshold & bobot |
| `src/test/scenarios/index.ts` | Daftar semua skenario |
