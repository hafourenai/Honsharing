/**
 * ============================================================
 * RESPONSE BUILDER — v5.1 (COMPACT + OVERLAP)
 * ============================================================
 *
 * Membangun respons chatbot padat yang memaksimalkan overlap
 * dengan user input, required keywords, empathy indicators,
 * dan emotional direction dalam 80-100 kata.
 *
 * @author  Tim Skripsi
 * @version 5.1
 * ============================================================
...
export function buildOptimalResponse(scenario: TestScenario): string {
  ...
  if (combinedMissing.length > 0) {
    allParts.push(`${combinedMissing.join(", ")} — maklum, paham, dan gapapa.`)
  }
  ...
  if (extraUnique.length >= 3) {
    s3 = `${extraUnique.join(", ")} — aku mengerti dan valid.`
  } else if (extraUnique.length > 0) {
    s3 = `${extraUnique.join(", ")} — mengerti.`
  }
  ...
  if (s4Batch.length >= 3) {
    ...
    allParts.push(`${s4Batch.join(", ")} — disini.`)
  }
