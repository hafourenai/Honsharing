...
// MOCK RETRIEVAL UNTUK MOCK MODE â€” PASTI ADA HASIL

/**
 * Retrieval untuk MOCK mode yang menjamin selalu mengembalikan
 * chunk yang relevan. Menggabungkan similarity-based retrieval
 * dengan fallback deterministik.
 *
 * @param query - Query user
 * @param expectedChunkIds - ID chunk yang diharapkan untuk skenario
 * @returns Array chunk dengan score
 */
export function mockRetrieveForScenario(
  query: string,
  expectedChunkIds: string[],
): (Chunk & { score: number })[] {
  // Coba similarity-based retrieval dulu
  const queryEmbedding = getMockEmbedding(query);

  const scored: (Chunk & { score: number })[] = ALL_MOCK_CHUNKS.map((chunk) => {
    const embedding =
      chunk.embedding ||
      getMockEmbedding(
        `${chunk.scenario.topic}. ${chunk.scenario.situation}. Kebutuhan: ${(chunk.metadata.need || []).join(", ")}. Emosi: ${(chunk.metadata.emotion || []).join(", ")}.`,
  ...
  const threshold = TEST_CONFIG.mockRetrieval.threshold;
  const topK = TEST_CONFIG.mockRetrieval.topK;

  const aboveThreshold = scored.filter((item) => item.score >= threshold);
  const topResults = aboveThreshold
    .sort((a, b) => b.score - a.score)
    .slice(0, topK);

  // Jika similarity-based gagal (< threshold), fallback ke deterministic
  if (topResults.length === 0) {
    return mockDeterministicRetrieval(expectedChunkIds);
  }
