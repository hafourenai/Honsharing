...
import type { EvaluationModeHandler } from "./evaluation-modes"
...
export async function runEvaluation(
  ...
  for (const scenario of singleTurnScenarios) {
    ...
    const logRetrievedContext: RetrievedChunkInfo[] = (retrievedContext as (Chunk & { score: number })[]).map((chunk, i) => ({
      chunkId: chunk.id,
    ...
    const entry = createLogEntry({
      ...
      retrievedContext: logRetrievedContext,
