...
export function compareRetrievalResults(
  ...
  if (commonTopics.length > 0) {
    lines.push(
      `**Topik yang sama:** ${commonTopics.join(", ")} â€” Menunjukkan konsistensi retrieval antar skenario.`,
    );
  } else {
    lines.push(
      "**Tidak ada topik yang sama** â€” Menunjukkan retrieval spesifik per skenario.",
    );
  }
  lines.push(``);
