...
export async function evaluateMultiTurnScenario(
  ...
  for (let i = 0; i < scenario.turns.length; i++) {
    ...
    if (mode.mode === "REAL") {
      const fakeScenario = { id: scenario.id, userInput: turn.userInput } as import("@test/types").TestScenario
      const result = await mode.getResponse(turn.userInput, fakeScenario)
