...
function detectGenericResponse(response: string): {
  ...
  if (matchedGeneric.length >= 1) {
    // Cek panjang respons â€” respons pendek + generic = sangat generik
...
export function generateFailureTable(analyses: FailureAnalysis[]): string {
  ...
  for (const a of analyses) {
    lines.push(
      `| ${a.scenarioName} | ${a.label} | ${a.qualityScore} | ${a.isIrrelevant ? "âš ï¸" : "âœ…"} | ${a.isWrongContext ? "âš ï¸" : "âœ…"} | ${a.isGenericResponse ? "âš ï¸" : "âœ…"} | ${a.isHallucination ? "âš ï¸" : "âœ…"} | ${a.isEmotionalMismatch ? "âš ï¸" : "âœ…"} |`,
    );
  }
  lines.push(``);
